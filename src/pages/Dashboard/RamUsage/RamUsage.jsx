...
const CpuUsage = (props) => {
  ...
  useEffect(() => {
    UsageCpuDashboardService(token)
      .then((res) => {
        const data = {
          free: 100 - res.data.ramUsageinPercent.replace('%', ''),
          used: parseInt(res.data.ramUsageinPercent.replace('%', '')),
        }
        if (data.used > 50) {
  ...
  return (
    <DashboardBox extraClasses='overflow-hidden'>
      <div className='flex justify-between'>
        <DashboardBoxHead text={` مقدار RAM مصرفی از (${totalRam})`} />
      </div>
      <div>
        {/* —————————— D A T E - R A N G E —————————— */}
